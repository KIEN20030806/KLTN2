...
def mode_0_program():
    asyncio.run(display.show_emotion("NEUTRAL"))
        ...
        if msg:
            if msg == b'END':
                ...
                for cmd in command_list:
                    ...
                    robot.move(cmd)
                command_list = []
                timeout_ms = 6000  # 1 giây
                start_time = time.ticks_ms()

                while True:
                    uid = rfid.scan_card()
                    if uid:
                        audio.play_w(rfid.uid_wav(uid))
                        break  # Có UID -> phát âm và thoát

                    if time.ticks_diff(time.ticks_ms(), start_time) > timeout_ms:
                        print("[INFO] Timeout: Không phát hiện UID trong 1 giây.")
    ...
    while True:
        ...
        if msg:
            try:
                ...
                if cmd[0] == '9':
                    asyncio.run(display.show_emotion(cmd))
...
while raw < 3800:
    asyncio.run(display.show_emotion("NEUTRAL"))
    raw = adc.read()  # Giá trị ADC từ 0–4095
asyncio.run(display.show_emotion("NEUTRAL", loop=False))
